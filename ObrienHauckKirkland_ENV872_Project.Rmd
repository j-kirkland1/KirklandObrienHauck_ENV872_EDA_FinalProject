...
packages <- c("tidyverse", "readxl", "dplyr", "lubridate", "sp", "sf","mapview",
              "knitr", "here", "ggplot2", "zoo", "trend", "RColorBrewer",
              "broom")
...
the largest run of Atlantic salmon on the East Coast (Natural Resources 
Council of Maine, 2013; The Nature Conservancy, n.d.). Despite its ecological 
...
production process, leaving 13 tons of Mercury in the river by 1970 (Island 
Institute, n.d.).
...
crab fisheries have been shut down (Maine Department of Environmental 
Protection, n.d.).
...
to pay $197 million in remediation for the mercury damage (A 22-Year Court 
Battle Ends with Justice for the Penobscot River, 2022; Penobscot River 
Remediation, n.d.).
...
studies of the Penobscot watershed (Island Institute, n.d.). Using this data, 
we will examine mercury levels in the sediment.
...
```{r setup.spatial.databases, echo=FALSE, message=FALSE, results='hide'}
...
representing the mercury probable effect concentration (PEC) 
(MacDonald et al., 2000), above which harmful effects to sediment-dwelling 
biota are likely to occur.


```{r Figure1, warning=FALSE, echo=FALSE, fig.cap='Concentrations of mercury in the Penobscot River by depth.'}

#Plot Depths and Concentration
figure1 <-ggplot(data_wrangle_2, aes(x=Date, y=Sample_Upper_Depth, 
                                      fill= cut(Max_Result_.Raw.,
  ...
  geom_point(shape=21) +
  scale_y_reverse() +
  labs(y="Depth (cm)", fill="Concentration (PPM)", x = "Year") +
...
```{r Figure2, warning=FALSE, echo=FALSE, fig.cap='Concentrations of mercury in the Penobscot River over time. The red line represents the mercury PEC, which is 1.06 PPM (MacDonald et al. 2000).'}
#Plot concentrations samples over time - gap between 1970 and mid 90s identified
figure2 <- ggplot(data_wrangle_2, aes(x=Date, y=Max_Result_.Raw.)) +
  geom_point() +
...
This first map shows mercury concentration, in part per million (PPM) 
as a color gradient in the spatially situated river. The highest measured 
concentrations reach 108.63 PPM, about 100 times the PEC of mercury in sediment, 
1.06 PPM. Because the PEC represents the threshold at which benthic 
macroinvertebrates are likely to experience toxic effects from mercury, these 
results emphasize how severely toxic the Penobscot River sediments are.


```{r map.1, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r map.2, warning=FALSE, message=FALSE, echo=FALSE}
...
We ran a multiple linear regression to determine if either 
variable, time or depth, had an impact on mercury concentration in the river. For this model, a P-value greater than the level of significance 0.05 implies that there is no statistically significant relationship, while a P-value less than 0.05 implies a statistically significant relationship.

```{r MLR.time.depth, warning=FALSE, message=FALSE, echo=FALSE}
...
This analysis starts with specific wrangling into a month column
...
see an unexpected peak in 2021.
```{r Figure3, echo=FALSE, fig.cap='Mean monthly concentration of mercury over time.'}
...
figure3 <- ggplot(MonthlyData_final,
        aes(x = Date, y = Mean_Conc)) +
  geom_line() +
  geom_smooth(method = "lm")+
  labs(
  title= "Mean Monthly Concentrations over Time (PPM)",
  x = "Date", 
...
```{r temporal.linear, echo=FALSE, message=FALSE}
#Linear model
...
```{r Figure4, echo=FALSE, fig.cap='Time-series of mercurcy concentration trends between 2000 and 2021.'}
...
```{r temporal.timeseries.seasonal, echo=FALSE}
...
```{r temporal.timeseries.nonseasonal, echo=FALSE}
...
```{r Figure5, echo=FALSE, fig.cap='Plot of concentrations over time and their depth category. Surface depth refers to data in the top four inches, while subsurface data is any depth below that.'}
...
```{r Figure6, echo=FALSE, fig.cap='Mercury concentrations by depth.'}
#simple plot
figure5 <- ggplot(DepthPlot.data,
...
```{r Figure7, echo=FALSE, fig.cap='Mercury concentrations by depth in the top two feet of benthic sediment.'}
...
figure6 <-ggplot(DepthPlot.data_bioactive,
       aes(x = Avg_Depth_ft, y = Max_Result_.Raw.)) +
  geom_point(alpha = 0.7, color = "darkblue") + 
  labs(
    title = "Mercury Concentration vs. Average Depth",
...
linear relationship, as the p-value of 0.0001124 was statistically significant 
being that it was < 0.05. 
...
https://github.com/j-kirkland1/KirklandObrienHauckOrtiz_ENV872_EDA_FinalProject
