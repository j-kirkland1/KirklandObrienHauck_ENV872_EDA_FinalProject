...
representing the mercury PEC (MacDonald et al. 2000), above which harmful effects to sediment-dwelling 
...
The results show that at least one predictor is associated with concentration, 
but it is small. The P-value is less than 0.05, so either time, depth, or both have some linear relationship with concentration. However, an R-squared value of <0.01 is very small, meaning the relationship is very weak, and the relationship is likely not significant. To further explore this relationship, we conducted linear regressions on each variable, time and depth, separately to see which one is positively correlated with concentration.
...
The P-value is greater than 0.05, therefore we can conclude that the relationship between time and the mean monthly concentration is not linear or statistically significant. 
...
seasonal patterns, we obtained a p-value of 0.596.

```{r temporal.timeseries, echo=FALSE}
...
This is not a significant p-value, indicating that there is no statistically significant trend in the seasonal data. We then ran a non-seasonal trend test, which also produced a non-significant p-value.
...
```{r Figure4, echo=FALSE, fig.cap='Plot of concentrations over time and their depth category. Surface depth refers to data in the top four inches, while subsurface data is any depth below that.'}
...
```{r Figure5, echo=FALSE, fig.cap='Mercury concentrations by depth.'}
#simple plot
...
```{r Figure6, echo=FALSE, fig.cap='Mercury concentrations by depth in the top two feet of benthic sediment.'}
...
# References
MacDonald, D. D., Ingersoll, C. G., & Berger, T. A. 2000. Development and Evaluation of Consensus-Based Sediment Quality Guidelines for Freshwater Ecosystems. Environmental Contamination and Toxicology. 39:20-31. https://doi.org/10.1007/s002440010075. 
